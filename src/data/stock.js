
export const stock = [
    {
        nombre: 'Botines adidas Copa Sense.3 Fg',
        id: 1,
        desc: 'Los botines adidas Copa Sense.3 FG están confeccionados en cuero sintético para una mayor resistencia y suavidad en tu contacto con la pelota. Su parte superior no posee costuras lo que te ofrece una mayor comodidad al momento de usarlos, mientras que el ajuste con cordones te ofrece el control a tu medida. Pensados para césped natural.',
        precio: 14499,
        img: 'https://celadasa.vteximg.com.br/arquivos/ids/193737-1000-1000/FY6196-1.jpg?v=637674864070700000',
        stock: 10,
        categoria:'botines'
    },
    {
        nombre: 'Botines Nike Legend 9 Elite Fg',
        id: 2,
        desc: 'Lorem ipsum producto 2',
        precio: 41999,
        img: 'https://www.dexter.com.ar/on/demandware.static/-/Sites-dabra-catalog/default/dw1181fd5d/products/NI_CZ8482-176/NI_CZ8482-176-1.JPG',
        stock: 20,
        categoria:'botines'
    },
    {
        nombre: 'Botines Nike Phantom Vision 2 Elite Dynamic Fit Fg',
        id: 3,
        desc: 'Lorem ipsum producto 3',
        precio: 49999,
        img: 'https://www.dexter.com.ar/on/demandware.static/-/Sites-dabra-catalog/default/dwc5604c4a/products/NI_CD4161-906/NI_CD4161-906-1.JPG',
        stock: 8,
        categoria:'botines'
    },
    {
        nombre: 'Botines Nike Superfly 7 Academy Mds Fg Mg',
        id: 4,
        desc: 'Lorem ipsum producto 4',
        precio: 15999,
        img: 'https://www.dexter.com.ar/on/demandware.static/-/Sites-dabra-catalog/default/dw4b544f71/products/NI_BQ5427-703/NI_BQ5427-703-1.JPG',
        stock: 7,
        categoria:'botines'
    },
    {
        nombre: 'Puma ultra 1.3 Mxsg',
        id: 5,
        desc: 'No te pierdas ni un segundo en la cancha y llevá a cabo esa jugada maestra que tenés en la mente con los Botines Puma Ultra 1.3 Mxsg que están diseñados sin concesiones para que te enfrentes a cualquier desafío que se presente en el campo.',
        precio: 26499,
        img: 'https://www.dexter.com.ar/on/demandware.static/-/Sites-dabra-catalog/default/dw6571eba8/products/PU_106513-01/PU_106513-01-1.JPG',
        stock: 23,
        categoria:'botines'
    },
    {
        nombre: 'Camiseta Puma Manchester City 2020/21',
        id: 6,
        desc: 'La camiseta Puma del Manchester City 2020/21 corresponde al conjunto titular que usan los Citizens en la Premier League como también en competencias internacionales. El diseño rinde homenaje a la ciudad y sus emblemáticos mosaicos del arte callejero. Fabricada con tecnología dryCELL para mantenerte seco y confortable hasta en los partidos más difíciles.',
        precio: 6899,
        img: 'https://www.dexter.com.ar/on/demandware.static/-/Sites-dabra-catalog/default/dw218dab59/products/PU_757058-01/PU_757058-01-1.JPG',
        stock: 12,
        categoria:'camisetas'
    },
    {
        nombre: 'Guantes Arquero Dribbling Royal 3.0',
        id: 7,
        desc: 'Los Guantes Arquero Dribbling Royal 3.0 fueron diseñados en latéx alemán de 3 mm y tela mesh logrando un ajuste más anatómico, con corte flat y finger safe extraíble para mayor protección. Ideales para darle un uso para competición.',
        precio: 6199,
        img: 'https://www.dexter.com.ar/on/demandware.static/-/Sites-dabra-catalog/default/dw3008d4bf/products/DG_DFIAGU009ACB/DG_DFIAGU009ACB-1.JPG',
        stock: 11,
        categoria:'accesorios'
    },
    {
        nombre: 'Camiseta Nike Chelsea Fc 2021/22 Stadium Away',
        id: 8,
        desc: 'La Camiseta Nike Chelsea Fc 2021/22 Stadium Away combina detalles del diseño tipo replica inspirado en tu equipo favorito y al llevarla puesta te sientas un profesional en el campo. Confeccionado en poliéster reciclado y con tecnología Nike Dri-FIT que absorbe el sudor de la piel para acelerar la evaporación, lo que ayuda a mantener la transpirabilidad y la comodidad.',
        precio: 9999,
        img: 'https://www.dexter.com.ar/on/demandware.static/-/Sites-dabra-catalog/default/dwae589c8d/products/NI_CV7888-732/NI_CV7888-732-1.JPG',
        stock: 14,
        categoria:'camisetas'
    },
    {
        nombre: 'Camiseta adidas Selección Argentina Alternativa 2021',
        id: 9,
        desc: 'La camiseta adidas de la Selección Argentina de fútbol (AFA) versión alternativa representa los colores de un país que vive para el fútbol y deja todo de lado cada vez que hay un Mundial, Copa América, Eliminatorias o un simple amistoso. Confeccionada en poliéster y con tecnología HEAT.RDY para mantenerte en alto rendimiento en todo momento. Usala cuando quieras, sentila siempre.',
        precio: 6999,
        img: 'https://www.dexter.com.ar/on/demandware.static/-/Sites-dabra-catalog/default/dw62bfdbd4/products/AD_FH8572/AD_FH8572-1.JPG',
        stock: 50,
        categoria:'camisetas'
    },
    {
        nombre: 'Camiseta Nike Liverpool Fc 2021/22 Stadium Home',
        id: 10,
        desc: 'Lorem ipsum producto 5',
        precio: 9999,
        img: 'https://www.dexter.com.ar/on/demandware.static/-/Sites-dabra-catalog/default/dwb23faaa4/products/NI_DB2560-688/NI_DB2560-688-1.JPG',
        stock: 8,
        categoria:'camisetas'
    },
    {
        nombre: 'Canilleras Umbro Neo Minimus Nc',
        id: 11,
        desc: 'Lorem ipsum producto 5',
        precio: 2449,
        img: 'https://www.dexter.com.ar/on/demandware.static/-/Sites-dabra-catalog/default/dw844681d8/products/UB_21120U-KPJ/UB_21120U-KPJ-2.JPG',
        stock: 10,
        categoria:'accesorios'
    },
    {
        nombre: 'Medias adidas 21',
        id: 12,
        desc: 'Perfectos para los más chicos que saben de grandeza, complementá el uniforme de tu hijo con el Short adidas River Plate en color rojo que demuestra la pasión y grandeza que sienten por su equipo del alma, las tres líneas tradicionales de la marca en negro y el escudo en el muslo derecho les da el estilo que todo fanático quiere lucir. Además, su material los hace suaves y frescos incluso en momentos de tensión.',
        precio: 1999,
        img: 'https://www.dexter.com.ar/on/demandware.static/-/Sites-dabra-catalog/default/dwad3e839c/products/AD_H18874/AD_H18874-1.JPG',
        stock: 12,
        categoria:'accesorios'
    },
    {
        nombre: 'Cinta Capitán Dribbling Basic',
        id: 13,
        desc: 'Llevá el mando del equipo y convertite en el líder ideal con la Cinta Capitán Dribbling Basic. Confeccionado en sintético, de color llamativo y con ajuste en velcro.',
        precio: 599,
        img: 'https://www.dexter.com.ar/on/demandware.static/-/Sites-dabra-catalog/default/dw92495812/products/DG_DVAVVZ002Y/DG_DVAVVZ002Y-1.JPG',
        stock: 10,
        categoria:'accesorios'
    }

]